# Developing a VS Code “Task Manager” Chat Extension
...
By now, our Task Manager extension can handle core task management via chat. The user can converse with `@taskmaster` to manage their project tasks in a natural way, and the agent uses VS Code’s own model integration to do the heavy lifting.
