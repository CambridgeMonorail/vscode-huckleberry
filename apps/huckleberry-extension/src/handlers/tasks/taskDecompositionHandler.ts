...
`;
            ...
            return parsedSubtasks
                .filter((subtask: any): boolean => {
                    return !!subtask && !!subtask.description && typeof subtask.description === 'string';
                })
                .map((subtask: any): { description: string; priority: TaskPriority } => {
                    // Ensure priority is never undefined by defaulting to the parent task's priority
                    const subtaskPriority = (subtask.priority && ['critical', 'high', 'medium', 'low'].includes(subtask.priority))
